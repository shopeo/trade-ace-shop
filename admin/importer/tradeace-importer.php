...
defined('TRADEACE_IMPORT_TOTAL') or define('TRADEACE_IMPORT_TOTAL', '25');

/**
 * Menu Importer Dashboard
 */
add_action('admin_menu', 'tradeace_data_importer_menu', 99);
function tradeace_data_importer_menu() {
    if (current_user_can('manage_options')) {
        $args = array(
            'parent_slug' => 'themes.php', // Parent Menu slug.
            'page_title' => esc_html__('Theme Setup', 'tradeace-theme'),
            'menu_title' => esc_html__('Theme Setup', 'tradeace-theme'),
            'capability' => 'edit_theme_options', // Capability.
            'menu_slug' => 'tradeace-install-demo-data', // Menu slug.
            'function' => 'tradeace_import_demo_data_output', // Callback.
        );
...
/**
 * Page Tradeace Importer
 */
function tradeace_import_demo_data_output() {
    wp_enqueue_script('tradeace_back_end-script-demo-data', TRADEACE_THEME_URI . '/admin/assets/js/tradeace-core-demo-data.js');
    $tradeace_core_js = 'var ajax_admin_demo_data="' . esc_url(admin_url('admin-ajax.php')) . '";';
    wp_add_inline_script('tradeace_back_end-script-demo-data', $tradeace_core_js, 'before');
    
    include TRADE_ACE_ADMIN_PATH . 'importer/tpl-import-demo-data.php';
}

/**
 * Install Child Theme
 */
add_action('wp_ajax_tradeace_install_child_theme', 'tradeace_install_child_theme');
function tradeace_install_child_theme() {
    ...
    $zip = TRADE_ACE_ADMIN_PATH . 'importer/theme-child/theme-child.zip';
    ...
    try {
        // unzip child-theme
        $theme_root = TRADEACE_THEME_PATH . '/../';
        $pathArrayString = str_replace(array('/', '\\'), '|', TRADEACE_THEME_PATH);
 ...
 * Install Plugin
 */
add_action('wp_ajax_tradeace_install_plugin', 'tradeace_install_plugin');
function tradeace_install_plugin() {
    ...
    if (trim($plugin_slug) !== '') {
        ...
        if (!class_exists('Trade_Ace_Auto_Install_Plugins')) {
            require_once TRADE_ACE_ADMIN_PATH . 'importer/auto-install-plugins.php';
        }
        
        $auto_install = new Trade_Ace_Auto_Install_Plugins($plugin_info);
        
        $res['mess'] = $plugin_info['name'];
        $res['status'] = $auto_install->tradeace_plugin_install() ? '1' : '0';
 ...
 * Import demo data
 */
add_action('wp_ajax_tradeace_import_contents', 'tradeace_import_contents');
function tradeace_import_contents() {
    set_time_limit(0);
    ...
    if (current_user_can('manage_options')) {
        ...
        if (!class_exists('WP_Import')) { // if WP importer doesn't exist
            $wp_import = TRADE_ACE_ADMIN_PATH . 'importer/wordpress-importer.php';
            require_once $wp_import;
        }

        if (class_exists('WP_Importer') && class_exists('WP_Import')) {
            if (!isset($_SESSION['tradeace_import']) || $partial == 1) {
            ...
            if (class_exists('WooCommerce')) {
                ...
                $theme_xml = TRADE_ACE_ADMIN_PATH . 'importer/data-import/datas/data_Part_0' . $partial . '_of_' . TRADEACE_IMPORT_TOTAL . '.xml';
                if (is_file($theme_xml)) {
                    ...
                    $res['mess'] = '<p class="tradeace-error">';
                    $res['mess'] .= 'file: ' . TRADE_ACE_ADMIN_PATH . 'importer/data-import/datas/data_Part_0' . $partial . '_of_' . TRADEACE_IMPORT_TOTAL . '.xml is not exists';
                    $res['mess'] .= '</p>';
                    $res['nofile'] = 'true';
                }
 ...
 */
if (isset($_REQUEST['action']) && 'tradeace_import_widgets_sidebar' == $_REQUEST['action']) {
    require_once TRADE_ACE_ADMIN_PATH . 'importer/tradeace-sidebars-widgets.php';
}
add_action('wp_ajax_tradeace_import_widgets_sidebar', 'tradeace_import_widgets_sidebar');
function tradeace_import_widgets_sidebar() {
    try {
        $widget_data = tradeace_sidebars_widgets_import();
 ...
 * Import Homes
 */
add_action('wp_ajax_tradeace_import_homes', 'tradeace_import_homes');
function tradeace_import_homes() {
    $elm_pages = $_POST['elm'];
    $wpb_pages = $_POST['wpb'];
    
    if (!class_exists('Trade_Ace_DF_Page_Importer')) {
        require_once TRADE_ACE_ADMIN_PATH . 'importer/tradeace-default-page.php';
    }
    ...
    try {
        ...
        if (!empty($elm_pages)) {
            ...
            foreach ($elm_pages as $file) {
                $file = trim($file);
                Trade_Ace_DF_Page_Importer::tradeace_push_data_from_file('elm', $file);
            }
        }
        ...
        if (!empty($wpb_pages)) {
            foreach ($wpb_pages as $file) {
                $file = trim($file);
                Trade_Ace_DF_Page_Importer::tradeace_push_data_from_file('wpb', $file);
            }
        }
    } catch (Exception $exc) {
        echo $exc->getMessage();
 ...
 * Import Revslider
 */
add_action('wp_ajax_tradeace_import_revsliders', 'tradeace_import_revsliders');
function tradeace_import_revsliders() {
    if (!class_exists('RevSliderSliderImport')) {
        die('0');
    }
    
    $zips = glob(TRADE_ACE_ADMIN_PATH . 'importer/data-import/revsliders/*.zip');
 ...
 * Global Options
 */
add_action('wp_ajax_tradeace_global_options', 'tradeace_global_options');
function tradeace_global_options() {
    /**
     * Setting Main Menu
     */
     ...
     */
    tradeace_update_ux_attrs();
    
    /**
     * Set Default Options
     */
    tradeace_theme_set_options_default();
 ...
 */
function tradeace_update_ux_attrs() {
    global $wpdb;
    ...
    $wpdb->update(
        $attrs_table,
        array('attribute_type' => 'tradeace_label', 'attribute_public' => 0),
    ...
    $wpdb->update(
        $attrs_table,
        array('attribute_type' => 'tradeace_color', 'attribute_public' => 0),
    ...
    if (!empty($terms)) {
        foreach ($terms as $term) {
            ...
            update_term_meta($term->term_id, 'tradeace_color', $color_code);
        }
    }
 ...
 * Set Default Options
 */
function tradeace_theme_set_options_default() {
    ...
    set_theme_mod('enable_tradeace_mobile', '1');
    set_theme_mod('single_product_mobile', '1');
    
    set_theme_mod('effect_before_load', '0');
    
    set_theme_mod('tradeace_cache_mode', 'file');
    set_theme_mod('tradeace_cache_expire', '36000'); // Cache live 10 hours
    
    update_option('yith_woocompare_compare_button_in_products_list', 'yes');
    update_option('woocommerce_enable_myaccount_registration', 'yes');
    
    if (function_exists('tradeace_theme_rebuilt_css_dynamic')) {
        tradeace_theme_rebuilt_css_dynamic();
    }
    ...
    delete_transient('_tradeace_products_deal');
    
    update_option('tradeacetheme_imported', 'imported');
}
