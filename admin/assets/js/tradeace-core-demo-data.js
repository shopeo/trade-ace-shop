 ...
 */
setTimeout(function() {
    if ($('.tradeace-tabs-heading').find('.tradeace-tab-heading').length) {
        $('.tradeace-tabs-heading').find('li:first-child .tradeace-tab-heading').trigger('click');
    }
    ...
    if ($('.builder-plugin').length <= 0) {
        $('.confirm-selected-plugins').removeClass('tradeace-disabled');
    } else {
        if ($('.builder-plugin').length === 2 && $('.builder-plugin.selected').length <= 0) {
            if (!$('.confirm-selected-plugins').hasClass('tradeace-disabled')) {
                $('.confirm-selected-plugins').addClass('tradeace-disabled');
            }
        } else {
            $('.confirm-selected-plugins').removeClass('tradeace-disabled');
        }
...
$('body').on('click', '.demo-homepage-item', function() {
    ...
    setTimeout(function() {
        if ($('.demo-homepage-item.selected').length) {
            if (!$('.tradeace-start-import').hasClass('selected')) {
                $('.tradeace-start-import').addClass('selected');
            }
        } else {
            $('.tradeace-start-import').removeClass('selected');
        }
    }, 10);
});
 ...
 * Tabs WPB - ELM
 */
$('body').on('click', '.tradeace-tab-heading', function() {
    var _this = $(this);
    
    if (!_clicking && !$(_this).hasClass('selected')) {
        _clicking = true;
        var _target = $(_this).attr('data-target');
        $('.tradeace-tabs-heading').find('.tradeace-tab-heading').removeClass('selected');
        $('.tradeace-tabs-panel').find('.demo-homepage-item-wrap').removeClass('tradeace-active');
        
        $('.tradeace-tabs-panel .tradeace-tab-content').removeClass('tradeace-show');
        if ($('.tradeace-tabs-panel').find(_target).length) {
            $('.tradeace-tabs-panel').find(_target).addClass('tradeace-show');
            $(_this).addClass('selected');
            
            setTimeout(function() {
                $('.tradeace-tabs-panel').find(_target + ' .demo-homepage-item-wrap').addClass('tradeace-active');
                _clicking = false;
            }, 300);
        }
    }
 ...
 * Click Demo Data
 */
$('body').on('click', '.tradeace-start-import.selected', function() {
    if (!$(this).hasClass('processing')) {
        _processing = true;
            
        $(this).addClass('processing');
        $('.runing-hide').hide();
        $('.tradeace-select-homepage').hide();
        $('.tradeace-start-import').hide();
        ...
        if ($('.recommend-plugin.selected').length) {
            $('.recommend-plugin.selected').each(function() {
                ...
                $('.plugins-installed').append('<li class="tradeace-label-plg tradeace-wait plg-' + _slug + '">' + _text + '</li>');
            });
        }

        _time_out = setInterval(function () {
            if (!_run) {
                tradeace_import_demo_data($);
...
function tradeace_install_child_theme($) {
    $.ajax({
        ...
        timeout: 600000,
        data: {
            action: 'tradeace_install_child_theme'
        },
 ...
 * @returns {undefined}
 */
function tradeace_install_plugins($) {
    if (!_installing) {
        _installing = true;
        var _this = $('.recommend-plugin.selected').first();
        var _slug = $(_this).attr('data-slug');
        if ($('.plugins-installed').find('.plg-' + _slug).length) {
            $('.plugins-installed').find('.plg-' + _slug).removeClass('tradeace-wait');
        ...
        $.ajax({
            ...
            timeout: 300000,
            data: {
                action: 'tradeace_install_plugin',
                plg: _slug
            },
            success: function (res) {
                $(_this).remove();
...
function tradeace_import_data($) {
    ...
    $.ajax({
        ...
        timeout: 300000,
        data: {
            'action': 'tradeace_import_contents',
            'file': _file
        },
        success: function (res) {
            if (_act >= _total) {
...
function tradeace_import_widgets_sidebar($) {
    $.ajax({
        ...
        timeout: 300000,
        data: {
            action: 'tradeace_import_widgets_sidebar'
        },
        success: function (res) {
            if (res === '1') {
...
function tradeace_import_revsliders($) {
    $.ajax({
        ...
        timeout: 300000,
        data: {
            action: 'tradeace_import_revsliders'
        },
        success: function (res) {
            if (res === '1') {
...
function tradeace_global_options($) {
    $.ajax({
        ...
        timeout: 300000,
        data: {
            action: 'tradeace_global_options'
        },
        success: function (res) {
            if (res === '1') {
                var permalink_url = $('.tradeace-start-import.selected').attr('data-permalink-option');
                $.ajax({
                    url: permalink_url,
                    type: 'get',
                    cache: false,
 ...
 * @returns {undefined}
 */
function tradeace_import_demo_data($) {
    var _step = $('.processing-steps li.runing').attr('data-step');
    if (_step) {
        switch (_step) {
            case '1':
                tradeace_install_child_theme($);
                break;
                
            case '2':
                tradeace_install_plugins($);
                break;
                
            case '3':
                tradeace_import_data($);
                break;
                
            case '4':
                tradeace_import_widgets_sidebar($);
                break;
                
            case '5':
                tradeace_import_homes($);
                break;
                
            case '6':
                tradeace_import_revsliders($);
                break;
                
            case '7':
                tradeace_global_options($);
                break;
