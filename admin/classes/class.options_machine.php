...
class Options_Machine {
                ...
                switch ($value['type']) {
                    ...
                    case 'typography':
                        ...
                        if (isset($typography_stored['size'])) {
                            $output .= '<div class="select_wrapper typography-size" original-title="' . esc_attr__('Font size', 'tradeace-theme') . '">';
                        ...
                        if (isset($typography_stored['face'])) {
                            ...
                            $faces = array(
                                'arial' => esc_html__('Arial', 'tradeace-theme'),
                                'verdana' => esc_html__('Verdana, Geneva', 'tradeace-theme'),
                                'trebuchet' => esc_html__('Trebuchet', 'tradeace-theme'),
                                'georgia' => esc_html__('Georgia', 'tradeace-theme'),
                                'times' => esc_html__('Times New Roman', 'tradeace-theme'),
                                'tahoma' => esc_html__('Tahoma, Geneva', 'tradeace-theme'),
                                'palatino' => esc_html__('Palatino', 'tradeace-theme'),
                                'helvetica' => esc_html__('Helvetica', 'tradeace-theme')
                            );
                        ...
                        if (isset($typography_stored['style'])) {
                            ...
                            $styles = array(
                                'normal' => esc_html__('Normal', 'tradeace-theme'),
                                'italic' => esc_html__('Italic', 'tradeace-theme'),
                                'bold' => esc_html__('Bold', 'tradeace-theme'),
                                'bold italic' => esc_html__('Bold Italic', 'tradeace-theme')
                            );
                    ...
                    case 'border':
                        ...
                        $styles = array(
                            'none' => esc_html__('None', 'tradeace-theme'),
                            'solid' => esc_html__('Solid', 'tradeace-theme'),
                            'dashed' => esc_html__('Dashed', 'tradeace-theme'),
                            'dotted' => esc_html__('Dotted', 'tradeace-theme')
                        );
                    ...
                    case 'button':
                        ...
                        $output .= '<img src="' . TRADEACE_THEME_URI . '/admin/assets/images/ajax-loader.gif" style="margin-left: 5px; display: none;" />';
                        break;
                    ...
                    case 'slider':
                        ...
                        $output .= '</ul>';
                        $output .= '<a href="#" class="button slide_add_button">' . esc_html__('Add New Slide', 'tradeace-theme') . '</a></div>';
                    ...
                    case 'backup':
                        ...
                        $backup = of_get_options(TRADEACE_ADMIN_BACKUPS);
                        $init = of_get_options('smof_init');

                        if (!isset($backup['backup_log'])) {
                            $log = esc_html__('No backups yet.', 'tradeace-theme');
                        } else {
                        ...
                        $output .= '<p><strong>' . esc_html__('Last Backup : ', 'tradeace-theme') . '<span class="backup-log">' . $log . '</span></strong></p></div>' . "\n";
                        $output .= '<a href="#" id="of_backup_button" class="button" title="' . esc_attr__('Backup Options', 'tradeace-theme') . '">' . esc_html__('Backup Options', 'tradeace-theme') . '</a>';
                    ...
                    case 'transfer':
                        ...
                        $output .= '<a href="#" id="of_import_button" class="button" title="' . esc_attr__('Import Options', 'tradeace-theme') . '">' . esc_html__('Import Options', 'tradeace-theme') . '</a>';
                    ...
                    // Tradeace upload Custom font
                    case 'tradeace_upload_custom_font' :
                        $output .= '<span><form action="" method="post">';
                        $output .= '<input type="hidden" name="action" value="wp_handle_upload" />';
                        $output .= '<input type="file" name="zip_packet_font" style="width: 230px" /><button style="margin: 0 0 0 5px; width: 100px" class="button-primary" type="submit" name="upload_new_font" value="new_font">' . esc_html__('Upload', 'tradeace-theme') . '</button>';
                        $output .= '</form></span>';
                        break;
                    ...
                    case 'switch':
                        ...
                        //Label ON
                        $on = !isset($value['on']) ? esc_html__('On', 'tradeace-theme') : $value['on'];

                        //Label OFF
                        $off = !isset($value['off']) ? esc_html__('Off', 'tradeace-theme') : $value['off'];
    ...
    public static function optionsframework_media_uploader_function($id, $std, $mod) {
        ...
        if (function_exists('wp_enqueue_media')) {
            $uploader .= '<a href="javascript:void(0);" class="button media_upload_button" id="' . $id . '">' . esc_html__('Upload', 'tradeace-theme') . '</a>';
            ...
            $uploader .= '<a href="javascript:void(0);"  class="button remove-image ' . $hide . '" id="reset_' . $id . '" title="' . $id . '">' . esc_html__('Remove', 'tradeace-theme') . '</a>';
        } else {
            $output .= '<p class="upload-notice"><i>' . esc_html__('Upgrade your version of WordPress for full media support.', 'tradeace-theme') . '</i></p>';
        }
    ...
    public static function optionsframework_slider_function($id, $std, $oldorder, $order) {
        ...
        if (!empty($val['title'])) {
            ...
            $slider .= '<li><div class="slide_header"><strong>' . esc_html__('Slide', 'tradeace-theme') . ' ' . $order . '</strong>';
        }

        $slider .= '<input type="hidden" class="slide of-input order" name="' . $id . '[' . $order . '][order]" id="' . $id . '_' . $order . '_slide_order" value="' . $order . '" />';

        $slider .= '<a class="slide_edit_button" href="#">' . esc_html__('Edit', 'tradeace-theme') . '</a></div>';

        $slider .= '<div class="slide_body">';

        $slider .= '<label>' . esc_html__('Title', 'tradeace-theme') . '</label>';
        ...
        $slider .= '<label>' . esc_html__('Image URL', 'tradeace-theme') . '</label>';
        $slider .= '<input class="upload slide of-input" name="' . $id . '[' . $order . '][url]" id="' . $id . '_' . $order . '_slide_url" value="' . $val['url'] . '" />';

        $slider .= '<div class="upload_button_div"><a href="javascript:void(0);" class="button media_upload_button" id="' . $id . '_' . $order . '">' . esc_html__('Upload', 'tradeace-theme') . '</a>';
        ...
        $slider .= '<a href="javascript:void(0);" class="button remove-image ' . $hide . '" id="reset_' . $id . '_' . $order . '" title="' . $id . '_' . $order . '">' . esc_html__('Remove', 'tradeace-theme') . '</a>';
        $slider .= '</div>' . "\n";
        ...
        $slider .= '</div>';
        $slider .= '<label>' . esc_html__('Link URL (optional)', 'tradeace-theme') . '</label>';
        $slider .= '<input class="slide of-input" name="' . $id . '[' . $order . '][link]" id="' . $id . '_' . $order . '_slide_link" value="' . $val['link'] . '" />';

        $slider .= '<label>' . esc_html__('Description (optional)', 'tradeace-theme') . '</label>';
