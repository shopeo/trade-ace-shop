...
jQuery(document).ready(function($) {
    ...
    $('body').on('tradeace_before_changed_src_main_img', function() {
        if (_zoom && $('.product-zoom .easyzoom').length) {
            if (!/Android|webOS|iPhone|iPad|iPod|BlackBerry/i.test(navigator.userAgent) && !iOS && $('.product-zoom .easyzoom .tradeace-disabled-touchstart').length <= 0) {
                $('.product-zoom .easyzoom').easyZoom();
            }
        }
    });
    
    $('body').on('tradeace_after_changed_src_main_img', function(ev, image_large, image_link) {
        // var _zoom = $('body').hasClass('product-zoom') ? true : false;
        ...
        if (_api_easyZoom) {
            ...
            var _imgChange = $('.main-images .tradeace-item-main-image-wrap[data-key="0"] img');
...
function reset_easy_zoom($) {
    var _zoom = $('body').hasClass('product-zoom') ? true : false;
    if (_zoom && !/Android|webOS|iPhone|iPad|iPod|BlackBerry/i.test(navigator.userAgent) && !iOS && $('.product-zoom .easyzoom .tradeace-disabled-touchstart').length <= 0) {
        if ($('.product-zoom .easyzoom').length > 0) {
            $('.product-zoom .easyzoom').each(function() {
                var _this = $(this);
                if (!$(_this).hasClass('tradeace-loaded')) {
                    ...
                    $(_this).addClass('tradeace-loaded');
                }
            });
        }
