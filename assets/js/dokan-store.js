...
if ($('.dokan-store-sidebar').length && $('.dokan-single-store').length) {
    ...
    $('.dokan-store-sidebar').prepend('<a href="javascript:void(0);" class="hidden-tag tradeace-close-sidebar"></a>');
    
    $('body').on('click', '.toggle-sidebar-dokan', function() {
        $('.black-window').fadeIn(200);
        if (!$('.dokan-store-sidebar').hasClass('tradeace-active')) {
            $('.dokan-store-sidebar').addClass('tradeace-active');
        }
    });
}
...
function popup_registration_dokan($) {
    ...
    if ($('input[name="tradeace-caching-enable"]').length && $('input[name="tradeace-caching-enable"]').val() === '1') {
        $('body').on('tradeace_after_load_static_content', function() {
            if ($('#tradeace-login-register-form form.register').length) {
