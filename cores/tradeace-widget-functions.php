...
/**
 * Content Tradeace Widget Filter Price - Slide
 */
if (!function_exists('tradeace_get_content_widget_price')) :

    function tradeace_get_content_widget_price($args = array(), $instance = array(), $show = true) {
        $args = array(
        ...
        $instance = array(
            'title' => isset($instance['title']) ? $instance['title'] : esc_html__('Price', 'tradeace-theme'),
        ...
        if (!is_shop() && !is_product_taxonomy()) {
            return 
                '<div id="' . $args['widget_id'] . '-ajax-wrap" class="tradeace-hide-price tradeace-filter-price-widget-wrap' . esc_attr($classWrap) . '">' .
        ...
        if ($min == $max) {
            return '<div id="' . $args['widget_id'] . '-ajax-wrap" class="tradeace-hide-price tradeace-filter-price-widget-wrap' . esc_attr($classWrap) . '">' .
                $args['before_title'] . $instance['title'] . $args['after_title'] .
            '</div>';
        }

        $res = '<div id="' . $args['widget_id'] . '-ajax-wrap" class="tradeace-filter-price-widget-wrap' . esc_attr($classWrap) . '">';
            ...
            '<form method="get" action="' . esc_url($form_action) . '">' .
                '<div class="price_slider_wrapper">' .
                    ...
                    '<div class="price_slider_amount">' .
                        '<input type="text" id="min_price" name="min_price" value="' . esc_attr($min_price) . '" data-min="' . esc_attr($data_min) . '" placeholder="' . esc_attr__('Min price', 'tradeace-theme') . '" />' .
                        '<input type="text" id="max_price" name="max_price" value="' . esc_attr($max_price) . '" data-max="' . esc_attr($data_max) . '" placeholder="' . esc_attr__('Max price', 'tradeace-theme') . '" />' .
                        '<div class="price_label">' .
                            esc_html__('Price:', 'tradeace-theme') . ' <span class="from"></span> &mdash; <span class="to"></span>' .
                        '</div>' .
                        $fields .
                        '<a href="javascript:void(0);" class="reset_price' . esc_attr($class_reset) . '" rel="nofollow">' . esc_html__('Reset', 'tradeace-theme') . '</a>' .
                        '<div class="tradeace-clear-both"></div>' .
                        ($instance['btn_filter'] ? '<button type="submit" class="button">' . esc_html__('Filter', 'tradeace-theme') . '</button>' : '') .
                    '</div>' .
                '</div>' .
                '<input type="hidden" class="tradeace_hasPrice" name="tradeace_hasPrice" value="' . esc_attr($hasPrice) . '" />' .
            '</form>';
 ...
 */
if (!function_exists('tradeace_get_filtered_price')) :
    function tradeace_get_filtered_price() {
        global $wpdb;
 ...
 */
if (!function_exists('tradeace_get_content_widget_variation')) :
    function tradeace_get_content_widget_variation($args, $instance) {
        ...
        $content = '<div id="' . esc_attr($args['widget_id']) . '-ajax-wrap" class="tradeace-filter-variations-widget-wrap">';
        ...
        if (0 < $count_terms) {
            $term_counts = tradeace_get_filtered_term_product_counts(wp_list_pluck($terms, 'term_id'), $taxonomy, $query_type);
            ...
            $label_switch = 'label';
            $image_switch = 'image';
            if ($tradeace_attr_ux_exist = class_exists('Tradeace_Abstract_WC_Attr_UX')) {
                $taxonomyObj = Tradeace_Abstract_WC_Attr_UX::get_tax_attribute($taxonomy);
                $color_switch = Tradeace_Abstract_WC_Attr_UX::_TRADEACE_COLOR;
                $label_switch = Tradeace_Abstract_WC_Attr_UX::_TRADEACE_LABEL;
                $image_switch = Tradeace_Abstract_WC_Attr_UX::_TRADEACE_IMAGE;
            }
            ...
            if ($taxonomyObj && isset($taxonomyObj->attribute_type)) {
                switch ($taxonomyObj->attribute_type) {
                    ...
                    case $image_switch:
                        ...
                        $tradeace_brands = isset($tradeace_opt['attr_brands']) && !empty($tradeace_opt['attr_brands']) ? $tradeace_opt['attr_brands'] : array();
                        $brands = array();
                        if (!empty($tradeace_brands)) {
                            foreach ($tradeace_brands as $key => $val) {
                                if ($val === '1') {
                                    $brands[] = $key;
                                }
                            }
                        ...
                        if (!empty($brands) && in_array($attr_name, $brands)) {
                            $class_ul = ' tradeace-variation-filters-brands small-block-grid-2 medium-block-grid-4 large-block-grid-6';
                            $brand_items = true;
                        }
            ...
            foreach ($terms as $k => $term) {
                ...
                $content_text = '<span class="tradeace-text-variation tradeace-text-variation-' . $vari_type . '">';
                ...
                $link = tradeace_get_page_base_url($taxonomy);
                ...
                if (isset($current_filter) && is_array($current_filter) && in_array($term->slug, $current_filter)) {
                    $class = ' chosen tradeace-chosen';
                    $aclass = ' tradeace-filter-var-chosen';
                } else {
                    $class = $aclass = '';
                }
                ...
                $countClss = 'count';
                if ($vari_type != '') {
                    $class .= ' tradeace-li-filter-' . $vari_type;
                    $aclass .= ' tradeace-filter-' . $vari_type;
                    $countClss .= ' tradeace-count-filter-' . $vari_type;
                }
                
                $aclass .= $brand_items ? ' tradeace-filter-brand-item' : '';

                if ($count) {
                    $hasResult = true;
                    $var_exist = true;
                } else if (!$count && $hide_empty) {
                    $class .= ' tradeace-empty-hidden';
                ...
                $liClass = ($k % 2 == 0) ? 'tradeace-odd' : 'tradeace-even';

                $liClass .= ' no-hidden';
                $style = ($vari_type == 'color') ? ' style="background:' . esc_attr($term_meta) . '"' : '';

                $liClass .= ($show_items > 0 && $k >= $show_items) ? ' tradeace-show-less' : '';

                $content_li .= '<li class="' . $liClass . $class . ' tradeace-attr-' . $attr . ' tradeace_' . $attr . '_' . esc_attr($term->term_id) . '">';

                $content_li .= 
                '<a ' .
                    'class="tradeace-filter-by-variations' . $aclass . '" ' .
                ...
                $content_li .= $vari_type == 'color' ? '<span class="tradeace-filter-color-span" ' . $style . '></span>' : '';

                if ($vari_type == 'image' && $tradeace_attr_ux_exist) {
                    if (class_exists('Tradeace_Abstract_WC_Attr_UX')) {
                        $image_size = $brand_items ? 'full' : 28;
                        $img_html = Tradeace_Abstract_WC_Attr_UX::get_image_preview($term_meta, false, $image_size, 28, $vari_txt);
                    } else {
                    ...
                    $content_li .= '<span class="tradeace-filter-image-span">' . $img_html . '</span>';
                }
            ...
            $content_ul = '<ul class="tradeace-variation-filters tradeace-variations-' . $vari_type . $class_ul . '">';

            $fadeIn = (isset($instance['effect']) && $instance['effect'] == 'fade') ? '1' : '0';
            $content_li .= ($show_items && ($count_terms > $show_items)) ?
                '<li class="tradeace_show_manual" data-fadein="' . $fadeIn . '">' .
                    '<a data-show="1" class="tradeace-show-more" href="javascript:void(0);" data-text="' . esc_attr__('- Show less', 'tradeace-theme') . '" rel="nofollow">' .
                        esc_html__('+ Show more', 'tradeace-theme') .
                    '</a>' .
                '</li>' : '';
 ...
 */
if (!function_exists('tradeace_get_filtered_term_product_counts')) :
    function tradeace_get_filtered_term_product_counts($term_ids, $taxonomy, $query_type) {
        global $wpdb;
 ...
 * @return int
 */
if (!function_exists('tradeace_get_current_term_slug')) :
    function tradeace_get_current_term_slug() {
 ...
 * @return string
 */
if (!function_exists('tradeace_get_page_base_url')) :
    function tradeace_get_page_base_url($taxonomy) {
         ...
         * Custom taxonomy
         */
        if (class_exists('Tradeace_WC_Taxonomy')) {
            $tradeace_taxonomy = apply_filters('tradeace_taxonomy_custom_cateogory', Tradeace_WC_Taxonomy::$tradeace_taxonomy);
            if (isset($_REQUEST[$tradeace_taxonomy])) {
                $link = add_query_arg($tradeace_taxonomy, wc_clean(wp_unslash($_REQUEST[$tradeace_taxonomy])), $link);
            }
        }
        ...
        if (class_exists('Trade_Ace_WC_Widget_Tags_Filter')) {
            if (isset($_REQUEST[Trade_Ace_WC_Widget_Tags_Filter::$_request_name]) && !empty($_REQUEST[Trade_Ace_WC_Widget_Tags_Filter::$_request_name])) {
                $link = add_query_arg(Trade_Ace_WC_Widget_Tags_Filter::$_request_name, rawurlencode(wc_clean($_REQUEST[Trade_Ace_WC_Widget_Tags_Filter::$_request_name])), $link);
            }
        }
        ...
        return apply_filters('tradeace_page_base_url', $link);
    }
endif;
 ...
 * @return string
 */
if (!function_exists('tradeace_get_origin_url')) :
    function tradeace_get_origin_url() {
        global $wp, $tradeace_origin_url;
        
        if (!isset($tradeace_origin_url)) {
            $tradeace_origin_url = get_option('permalink_structure') == '' ?
            ...
            $GLOBALS['tradeace_origin_url'] = $tradeace_origin_url;
        }
        
        return apply_filters('tradeace_origin_url', $tradeace_origin_url);
    }
endif;
 ...
 * @return string
 */
if (!function_exists('tradeace_get_origin_url_paging')) :
    function tradeace_get_origin_url_paging() {
        global $wp, $tradeace_origin_url_paging;
        
        if (!isset($tradeace_origin_url_paging)) {
            $tradeace_origin_url_paging = get_option('permalink_structure') == '' ?
                add_query_arg($wp->query_string, '', home_url($wp->request . '/')) :
                home_url($wp->request . '/');

            $GLOBALS['tradeace_origin_url_paging'] = $tradeace_origin_url_paging;
        }
        
        return apply_filters('tradeace_origin_url_paging', $tradeace_origin_url_paging);
    }
endif;
 ...
 */
// add_action('init', 'tradeace_remove_default_wg_woo');
if (!function_exists('tradeace_remove_default_wg_woo')) :
    function tradeace_remove_default_wg_woo() {
        global $tradeace_opt;
