...
echo apply_filters(
    ...
    sprintf(
        ...
        isset($class_button) ? esc_attr($class_button) : esc_attr(isset($args['class']) ? $args['class'] : 'add-to-cart-grid btn-link tradeace-tip'),
        esc_attr($product->get_type()), //product type
        esc_attr($title),
        $attributes,
        $title,
        isset($icon_class) ? esc_attr($icon_class) : 'tradeace-df-plus'
    ),
    $product
);
