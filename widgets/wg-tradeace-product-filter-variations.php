...
if (TRADEACE_WOO_ACTIVED) {
    add_action('widgets_init', 'tradeace_product_variations_widget');
    function tradeace_product_variations_widget() {
        register_widget('Trade_Ace_Product_Variations_Widget');
    }
     ...
     * @extends  WC_Widget
     */
    class Trade_Ace_Product_Variations_Widget extends WC_Widget {

        public static $tradeace_widget_id = 'tradeace_woocommerce_filter_variations';

        /**
         * Constructor
         */
        public function __construct() {
            $this->widget_cssclass = 'woocommerce widget_layered_nav tradeace-widget-has-active';
            $this->widget_description = esc_html__('Shows a custom attribute in a widget which lets you narrow down the list of products when viewing product categories.', 'tradeace-theme');
            $this->widget_id = self::$tradeace_widget_id;
            $this->widget_name = esc_html__('Tradeace Product Attributes Filter', 'tradeace-theme');
        ...
        public function form($instance) {
            ...
            echo "<p class='tradeace-widget-instance' data-instance='" . esc_attr(json_encode($instance)) . "'></p>";
        ...
        public function init_settings($instance) {
            ...
            $this->settings = array(
                'title' => array(
                    'type' => 'text',
                    'std' => esc_html__('Filter by', 'tradeace-theme'),
                    'label' => esc_html__('Title', 'tradeace-theme')
                ),
                'attribute' => array(
                    'type' => 'select',
                    'class' => 'tradeace-select-attr',
                    'std' => '',
                    'label' => esc_html__('Attribute', 'tradeace-theme'),
                    'options' => $attribute_array
                ),
                'query_type' => array(
                    'type' => 'select',
                    'std' => 'or',
                    'label' => esc_html__('Query type', 'tradeace-theme'),
                    'options' => array(
                        'or' => esc_html__('OR', 'tradeace-theme'),
                        'and' => esc_html__('AND', 'tradeace-theme')
                    )
                ),
                'show_items' => array(
                    'type' => 'text',
                ...
                'effect' => array(
                    'type' => 'select',
                    'std' => 'slide',
                    'label' => esc_html__('Effect for show more and show less', 'tradeace-theme'),
                    'options' => array(
                        'slide' => esc_html__('Slide Up-Down', 'tradeace-theme'),
                        'fade' => esc_html__('Fade In-Out', 'tradeace-theme')
                    )
                ),
                'hide_empty' => array(
                    'type' => 'checkbox',
                    'std' => 0,
                    'label' => esc_html__('Hide empty', 'tradeace-theme')
                ),
                'count' => array(
                    'type' => 'checkbox',
                    'std' => 0,
        ...
        public function widget($args, $instance) {
            ...
            // Get widget content
            $widget_data = tradeace_get_content_widget_variation($args, $instance);
